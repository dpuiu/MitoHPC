...
if [ -s $HP_RDIR/RNR.bed.gz ]   ; then bcftools annotate -a $HP_RDIR/RNR.bed.gz   $O.gz -c "CHROM,FROM,TO,RNR"  -h <(echo '##INFO=<ID=RNR,Number=0,Type=Flag,Description="RNR">')         > $O ; bgzip -f $O ; tabix -f $O.gz ; fi
if [ -s $HP_RDIR/TRN.bed.gz ]   ; then bcftools annotate -a $HP_RDIR/TRN.bed.gz   $O.gz -c "CHROM,FROM,TO,TRN"  -h <(echo '##INFO=<ID=TRN,Number=0,Type=Flag,Description="TRN">')         > $O ; bgzip -f $O ; tabix -f $O.gz ; fi
